...
class MoveBaseClient:
    def __init__(self):
        ...
        self.first_move_reach_goal = False
        self.second_move_reach_goal = False

        self.move_base_goal = MoveBaseGoal()
        self.go_to_docking = DockingClient()

    def first_move_to_goal(self) :
                   
        if not self.first_move_reach_goal:
            print("First MoveBase action start")

            self.move_base_goal.target_pose.header.frame_id = "map"
            self.move_base_goal.target_pose.header.stamp = rospy.Time.now()                 

            # target_pose should be changed whenver use this code
            self.move_base_goal.target_pose.pose.position.x = 1.0049941539764404
            self.move_base_goal.target_pose.pose.position.y = -0.005708754062652588
            self.move_base_goal.target_pose.pose.position.z = 0
            
            self.move_base_goal.target_pose.pose.orientation.x = 0
            self.move_base_goal.target_pose.pose.orientation.y = 0
            self.move_base_goal.target_pose.pose.orientation.z = -0.9999863695455604
            self.move_base_goal.target_pose.pose.orientation.w = 0.005221180239162203
            ...
            if self.client.get_result():
                print("First MoveBase_Action Complete!!")
                
                self.first_move_reach_goal = True
                self.go_to_docking.move_to_safe()

        def second_move_to_goal(self) :
                   
            if not self.second_move_reach_goal:
                print("Second MoveBase action start")

                self.move_base_goal.target_pose.header.frame_id = "map"
                self.move_base_goal.target_pose.header.stamp = rospy.Time.now()

                self.move_base_goal.target_pose.pose.position.x = 3.0
                self.move_base_goal.target_pose.pose.position.y = 0.0
                ...
                self.move_base_goal.target_pose.pose.orientation.z = 0.0
                self.move_base_goal.target_pose.pose.orientation.w = 0.0
                ...
                if self.client.get_result():
                    print("Second MoveBase_Action Complete!!")
                    
                    self.second_move_reach_goal = True
                    self.go_to_docking.move_to_safe()
...
class MultiClient :
    ...
    def Cycle(self):
        ...
        if not self._is_reach_multi_goal :
            self.move_base_result.first_move_to_goal()

            # when below code is True, it will perform
            if self.docking_result.move_to_safe() :

                self.move_base_result.second_move_reach_goal()
                print(self.move_base_result.second_move_reach_goal())
